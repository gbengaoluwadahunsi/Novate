...
class MedicalTranscriptParser {
  ...
  extractPastMedicalHistory(text) {
    ...
    return histories.length > 0 ? histories.join('; ') : 'Not recorded';
  }
  ...
  extractInvestigations(text) {
    ...
    return investigations.length > 0 ? investigations.join('; ') : 'Not recorded';
  }
  ...
  extractAllergies(text) {
    ...
    return allergies.length > 0 ? allergies.join('; ') : 'Not recorded';
  }
  ...
  calculateConfidence(extractedValue, patterns, text) {
    ...
    if (extractedValue !== 'Not recorded' && extractedValue !== 'Not extracted') {
      confidence += 30;
    }
...
export const calculateOverallConfidence = (data) => {
  ...
  const checkField = (value) => {
    totalFields++;
    if (value && value !== 'Not recorded' && value !== 'Not extracted' && value !== '[To be extracted from transcript]') {
      extractedFields++;
    }
  };
