// Utility to convert mapped JSON coordinates to medical-coordinates format
// This converts pixel coordinates to normalized coordinates (0-1 range)
...
interface MappedImageData {
  image_name: string
  image_type: string
  total_regions: number
  coordinates: Record<string, MappedCoordinate>
}

interface MappedJsonData {
  timestamp: string
  total_images: number
  mapped_images: number
  coordinates_by_image: Record<string, MappedImageData>
}
...
const IMAGE_WIDTH = 800  // Adjust based on your actual image width
const IMAGE_HEIGHT = 600 // Adjust based on your actual image height

// Convert pixel coordinates to normalized coordinates (0-1 range)
function convertToNormalized(pixelX: number, pixelY: number): { x: number; y: number } {
  return {
...
const ANATOMICAL_MAPPING: Record<string, { region: string; aliases?: string[] }> = {
...
export function convertMappedJsonToCoordinates(mappedData: MappedJsonData, imageName: string): BodyPartCoordinate[] {
  const imageData = mappedData.coordinates_by_image[imageName]
  if (!imageData) {
    throw new Error(`Image ${imageName} not found in mapped data`)
  }
  
  const coordinates: BodyPartCoordinate[] = []
...
// Example usage:
export function generateCardioRespiratoryCoordinates(mappedData: MappedJsonData): string {
  const coordinates = convertMappedJsonToCoordinates(mappedData, 'respicardio.png')
  
  // Generate TypeScript code
  let code = '// Cardiorespiratory System Coordinates (Gender Neutral) - Generated from mapped data\n'
